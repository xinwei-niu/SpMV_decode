...
export HF_MODEL_ID="${HF_MODEL_ID:-Qwen/Qwen3-14B}"

# Local Qwen3-14B checkpoint directory. Override with MODEL_DIR when needed.
export MODEL_DIR="${MODEL_DIR:-$ROOTDIR/models/qwen3-14b/Qwen/Qwen3-14B}"

# Supported by the Qwen3-14B benchmark. The benchmark itself decides how
...
case "$BENCH_MODE" in
  fixed)
    ENTRY="${ENTRY:-bench_qwen_31_14b_fp16_fixed_ptx.py}"
    ;;
  paired)
    ENTRY="${ENTRY:-bench_qwen3_14b_fp16_split_ratios.py}"
    ;;
  *)
...
OUTDIR="${OUTDIR:-$ROOTDIR/runs/qwen3_14b_triton_ptx_fp16_fixed_$(date -u +%Y%m%d_%H%M%S)_$$}"
...
print()
print("=" * 84)
print("QWEN3-14B FIXED-CONTEXT CUTE/PTX SUMMARY")
print("=" * 84)
print(
    f"{'ratio':<12}"
    f"{'median_ms':>16}"
