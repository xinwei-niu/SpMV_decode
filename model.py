...
NO_FLASH = _os.environ.get("E2E_NO_FLASH", "0") == "1"
SDPA_MEM = _os.environ.get("E2E_SDPA_MEM", "0") == "1"
if NO_FLASH:
    torch.backends.cuda.enable_flash_sdp(False)
    torch.backends.cuda.enable_mem_efficient_sdp(SDPA_MEM)
...
transformer_configs = {
    ...
    "qwen2.5-72b": dict(block_size=131072, n_layer=80, n_head=64, n_local_heads=8, dim=8192, intermediate_size=29568, vocab_size=152064, rope_base=1000000, norm_eps=1e-5),
...
class Transformer(nn.Module):
    ...
    def forward(self, mask: Optional[BlockMask], idx: Tensor, input_pos: Optional[Tensor] = None) -> Tensor:
        assert self.freqs_cis is not None, "Caches must be initialized first"
        if mask is not None and not NO_FLASH:
...
class TransformerBlock(nn.Module):
    ...
    def forward(self, x: Tensor, input_pos: Tensor, freqs_cis: Tensor, mask: Optional[BlockMask]) -> Tensor:
...
class Attention(nn.Module):
    ...
    def forward(self, x: Tensor, freqs_cis: Tensor, mask: Optional[BlockMask], input_pos: Optional[Tensor] = None) -> Tensor:
        ...
        if NO_FLASH:
            kv_len = k.shape[-2]
            kv_idx = torch.arange(kv_len, device=q.device)
            q_pos = input_pos.reshape(-1)
            attn_mask = (kv_idx[None, :] <= q_pos[:, None])[None, None]
            y = _F.scaled_dot_product_attention(q, k, v, attn_mask=attn_mask, enable_gqa=(self.n_head != self.n_local_heads))
        else:
